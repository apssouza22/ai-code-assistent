...
class UserInputAction(Action):
    ...
    prompt: str = Field(..., min_length=1, description="Message/question to show to the user")
    options: List[str] = Field(default_factory=list, description="Optional list of choices for the user to select from")
    allow_empty: bool = Field(False, description="Whether to allow empty input")
    timeout_secs: int = Field(60, gt=0, le=300, description="Timeout in seconds waiting for user input")
